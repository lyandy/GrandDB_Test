...
- (NSArray *)objectsOfClass:(Class<GYModelObjectProtocol>)modelClass
    ...
    NSString *columnSql = @"*";
    if ([modelClass andy_db_fts] && [modelClass andy_db_primaryKey] && !properties) {
...
- (id)valueForClass:(Class<GYModelObjectProtocol>)modelClass property:(NSString *)property resultSet:(FMResultSet *)resultSet index:(int)index {
    ...
    if (propertyType == GYPropertyTypeRelationship) {
        propertyClass = [[modelClass andy_db_propertyClasses] objectForKey:property];
        propertyType = [[[propertyClass andy_db_propertyTypes] objectForKey:[propertyClass andy_db_primaryKey]] unsignedIntegerValue];
    }
    ...
    if (![self needSerializationForType:propertyType]) {
        ...
        if (data.length) {
            if (propertyType == GYPropertyTypeTransformable) {
                Class propertyClass = [[modelClass andy_db_propertyClasses] objectForKey:property];
    ...
    if (propertyClass) {
        ...
        if (!cache) {
            ...
            [cache setValue:value forKey:[propertyClass andy_db_primaryKey]];
...
- (void)getSql:(NSString **)sql properties:(NSArray **)properties forClass:(Class<GYModelObjectProtocol>)modelClass withPrimaryKey:(BOOL)hasPrimaryKey {
    ...
    if (!hasPrimaryKey && [modelClass andy_db_primaryKey]) {
        [tempProperties removeObject:[modelClass andy_db_primaryKey]];
    }
...
- (BOOL)isPrimaryKeyProvidedWithObject:(id<GYModelObjectProtocol>)object {
    NSString *primaryKey = [[object class] andy_db_primaryKey];
    if (!primaryKey) {
        return YES;
    }
    
    id value = [(id)object valueForKey:primaryKey];
    GYPropertyType propertyType = [[[[object class] andy_db_propertyTypes] objectForKey:primaryKey] unsignedIntegerValue];
...
- (id)valueOfProperty:(NSString *)property ofObject:(id)modelObject {
    ...
    if (propertyType == GYPropertyTypeRelationship) {
        ...
        if (value) {
            Class propertyClass = [[modelClass andy_db_propertyClasses] objectForKey:property];
            value = [value valueForKey:[propertyClass andy_db_primaryKey]];
        }
...
- (void)inTransaction:(dispatch_block_t)block
    ...
    GYDatabaseInfo *databaseInfo = [self databaseInfoForDbName:dbName fullPath:[modelClass andy_db_fullPath]];
    [databaseInfo.databaseQueue syncInDatabase:^(FMDatabase *db) {
        if (databaseInfo.timer) {
...
- (GYDatabaseInfo *)databaseInfoForClass:(Class<GYModelObjectProtocol>)modelClass {
    GYDatabaseInfo *databaseInfo = [self databaseInfoForDbName:[modelClass andy_db_dbName] fullPath:[modelClass andy_db_fullPath]];
    
    @synchronized(databaseInfo.updatedTables) {
        if (![databaseInfo.updatedTables containsObject:[modelClass andy_db_tableName]]) {
            if ([self isTable:[modelClass andy_db_tableName] existsWithDatabaseQueue:databaseInfo.databaseQueue]) {
            ...
            [databaseInfo.updatedTables addObject:[modelClass andy_db_tableName]];
        }
    }
...
- (void)updateTableSchemaForClass:(Class<GYModelObjectProtocol>)modelClass
    ...
    NSArray *existingColumns = [self columnsForTable:[modelClass andy_db_tableName] databaseQueue:databaseQueue];
...
- (void)updateIndicesForClass:(Class<GYModelObjectProtocol>)modelClass
                databaseQueue:(FMDatabaseQueue *)databaseQueue {
    if (![(Class)modelClass respondsToSelector:@selector(andy_db_indices)])
        return;
    
    NSMutableSet *existingIndices = [self indicesForTable:[modelClass andy_db_tableName] databaseQueue:databaseQueue];
    
    for (NSArray *index in [modelClass andy_db_indices]) {
        NSString *indexName = [self indexNameForTable:[modelClass andy_db_tableName] properties:index];
...
- (NSString *)sqlForCreateVirtualTableForClass:(Class<GYModelObjectProtocol>)modelClass {
    ...
    [columns removeObject:[GYDCUtilities columnForClass:modelClass property:[modelClass andy_db_primaryKey]]];
    if ([(Class)modelClass respondsToSelector:@selector(andy_db_tokenize)]) {
        NSString *tokenize = [modelClass andy_db_tokenize];
        if (tokenize.length) {
...
- (NSArray *)sortedPropertiesForClass:(Class<GYModelObjectProtocol>)modelClass {
    NSString *primaryKey = [modelClass andy_db_primaryKey];
...
- (NSString *)columnDefinitionForClass:(Class<GYModelObjectProtocol>)modelClass property:(NSString *)property {
    ...
    if (propertyType == GYPropertyTypeRelationship) {
        ...
        if (primaryKey) {
            propertyType = [[[propertyClass andy_db_propertyTypes] objectForKey:primaryKey] unsignedIntegerValue];
        } else {
    ...
    if ([self mapsIntegerForType:propertyType]) {
        if ([property isEqualToString:[modelClass andy_db_primaryKey]]) {
    ...
    if ([(Class)modelClass respondsToSelector:@selector(andy_db_defaultValues)]) {
...
- (void)createIndicesForClass:(Class<GYModelObjectProtocol>)modelClass
    ...
    NSArray *indices = [modelClass andy_db_indices];
...
- (void)createIndexForClass:(Class<GYModelObjectProtocol>)modelClass
    ...
    [databaseQueue asyncInDatabase:^(FMDatabase *db) {
        ...
        NSString *sql = [[NSString alloc] initWithFormat:@"CREATE INDEX IF NOT EXISTS %@ ON %@ (%@)", [self indexNameForTable:[modelClass andy_db_tableName] properties:properties], [modelClass andy_db_tableName], columns];
        [db executeUpdate:sql];
    }];
}

- (BOOL)isVirtualTableForClass:(Class<GYModelObjectProtocol>)modelClass {
    if ([modelClass andy_db_fts].length) {
        return YES;
    } else {
        return NO;
    }
...
- (GYDatabaseInfo *)databaseInfoForDbName:(NSString *)dbName fullPath:(NSString *)fullPath {
    @synchronized(_databaseInfos) {
        GYDatabaseInfo *databaseInfo = [_databaseInfos objectForKey:dbName];
        if (!databaseInfo) {
            ...
            if (fullPath == nil)
            {
                fullPath = [self pathForDbName:dbName];
            }

            databaseInfo.databaseQueue = [FMDatabaseQueue databaseQueueWithPath:fullPath];
